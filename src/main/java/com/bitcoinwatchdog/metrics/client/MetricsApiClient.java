...
public class MetricsApiClient {
	...
	public List<PuellMultiple> getPuellMultiple(LocalDate date) {
		...
		List<PuellMultiple> response = webClient.get().uri(url).retrieve()
	...
	public List<NUPL> getNUPL(LocalDate date) {
		...
		List<NUPL> response = webClient.get().uri(url).retrieve()
	...
	public List<MVRVZScore> getMVRVZ(LocalDate date) {
		...
		List<MVRVZScore> response = webClient.get().uri(url).retrieve()
	...
	private String buildUrl(String endpoint, LocalDate date) {
		return UriComponentsBuilder.fromUriString(baseUrl + endpoint).queryParam("startday", date.toString())
				.queryParam("endday", date.toString()).toUriString();
	}
}
